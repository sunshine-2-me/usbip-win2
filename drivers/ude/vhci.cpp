...
PAGED auto initialize(_Inout_ WDFDEVICE_INIT *init)
{
        ...
        // Per-user isolation: only LocalSystem and Built-in Administrators may open
        // the VHCI control device. End-user RDP sessions must go through usbip-broker.
        DECLARE_CONST_UNICODE_STRING(sddl_admin_only, L"D:P(A;;GA;;;SY)(A;;GA;;;BA)");

        if (auto err = WdfDeviceInitAssignSDDLString(init, &sddl_admin_only)) {
