        ...
        case SERVICE_CONTROL_SESSIONCHANGE:
                if (g_ownership && data &&
                    (evt == WTS_SESSION_LOGOFF || evt == WTS_REMOTE_DISCONNECT)) {
                        ...
                        auto removed = g_ownership->release_by_session(notif->dwSessionId);
                        auto attach_removed = AttachState::instance().erase_by_session(notif->dwSessionId);
                        if (removed) {
                                spdlog::info("released {} ownership record(s) for session {}",
                                             removed, notif->dwSessionId);
                        }
                        if (attach_removed) {
                                spdlog::info("released {} attach-state record(s) for session {}",
                                             attach_removed, notif->dwSessionId);
                        }
                }
                return NO_ERROR;
